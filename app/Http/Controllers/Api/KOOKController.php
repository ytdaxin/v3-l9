...
class KOOKController extends Controller
{
    public function _webHookApi(Request $request)
    {
        ...
        $sn = $Bot['sn'] ?? 0;
        Log::channel('mylog')->info('1.>>> | '.oToJson($sn).oToJson($Bot));
        $oSn = $kv->get_data($dbSn); // 取回 事件唯一ID
        Log::channel('mylog')->info('2.>>> | '.oToJson($oSn));
        $robot = $Bot['robot'] ?? null;
        $data = $Bot['d'] ?? null; // 赋值 $post['d'] 下的成员数据 到 $data
        Log::channel('mylog')->info('3.>>> | '.oToJson($data));
...
//        }
        ...
        $json = json_encode([ // 构造 webHook 回调验证
            'challenge' => $challenge
        ]);
        Log::channel('mylog')->info('000.>>> | '.oToJson($json));
        return $json;
    }
}
